<template>
    <div class="yh-label-input" :style="{height:height,width:width}" @click="clickInput">
        <div class="yh-tag-box"  ref="tagBox">
            <el-tag v-for="(tag,index) in tags" :key="index" closable @close="delTag(index)"  class="yh-tag">{{ tag }}</el-tag>
        </div>
        <el-input v-model.trim="inputData" ref="testInput" @keydown.enter="addTag" class="yh-tag-input" :maxlength="maxTag"></el-input>
    </div>
</template>
